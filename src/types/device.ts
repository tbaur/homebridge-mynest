...
export type NestDevice =
  | { readonly identity: DeviceIdentity & { kind: 'thermostat' }, readonly state: ThermostatState }
  | { readonly identity: DeviceIdentity & { kind: 'protect' }, readonly state: ProtectState }
  | {
    readonly identity: DeviceIdentity & { kind: 'temperature_sensor' }
    readonly state: TemperatureSensorState
  }

/** One device of a specific kind, with the matching state type. */
export type DeviceOfKind<K extends DeviceKind> = Extract<NestDevice, { identity: { kind: K } }>
 ...
 */
export function isDeviceOfKind<K extends DeviceKind>(
  device: NestDevice,
  kind: K,
): device is DeviceOfKind<K> {
  return device.identity.kind === kind
}

/** The whole home, as the platform sees it after merging both transports. */
export interface DeviceInventory {
  readonly thermostats: ReadonlyMap<string, Extract<NestDevice, { identity: { kind: 'thermostat' } }>>
  readonly protects: ReadonlyMap<string, Extract<NestDevice, { identity: { kind: 'protect' } }>>
  readonly temperatureSensors: ReadonlyMap<
    string,
